...
func (d *Database) SaveMetrics(m *models.Metrics) error {
	_, err := d.db.Exec(`
		INSERT INTO metrics (agent_id, cpu_percent, memory_used, memory_total,
			disk_used, disk_total, network_rx, network_tx, load_avg_1, load_avg_5, load_avg_15, created_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		m.AgentID, m.CPUPercent, m.MemoryUsed, m.MemoryTotal,
...
func (d *Database) GetMetricsHistory(agentID string, since time.Time) ([]*models.Metrics, error) {
	rows, err := d.db.Query(`
		SELECT agent_id, cpu_percent, memory_used, memory_total,
			disk_used, disk_total, network_rx, network_tx, load_avg_1, load_avg_5, load_avg_15, created_at
		FROM metrics
	...
	for rows.Next() {
		m := &models.Metrics{}
		err := rows.Scan(&m.AgentID, &m.CPUPercent, &m.MemoryUsed, &m.MemoryTotal,
			&m.DiskUsed, &m.DiskTotal, &m.NetworkRx, &m.NetworkTx,
			&m.LoadAvg1, &m.LoadAvg5, &m.LoadAvg15, &m.Timestamp)
		if err != nil {
			return nil, err
